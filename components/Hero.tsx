...
export const Hero: React.FC<HeroProps> = ({ onReady, isIntroComplete, openWaitlist }) => {
  ...
  useEffect(() => {
    let active = true;
    const sequenceTimings = [2000, 2000, 2200, 1800];
    
    const driveTimeline = async () => {
      // Stage 0 -> 1 -> 2 -> 3 -> 4
      for (let i = 0; i < 4; i++) {
        if (!active) return;
        await new Promise(resolve => setTimeout(resolve, sequenceTimings[i]));
        if (!active) return;
        setStage(prev => prev + 1);
      }
      // Stage 4 reached. The tagline will trigger showCTA on completion.
      if (!active) return;
    };
  ...
  const sentenceVariants = {
    ...
    visible: {
      ...
      transition: {
        ...
        staggerChildren: 0.1, // Rhythmic word stagger (increased for pacing)
      },
    },
  };
  ...
  return (
    <section
      ...
      className={`relative ${isIntroComplete ? 'min-h-screen py-24' : 'h-[100dvh] overflow-hidden'} flex flex-col justify-center items-center bg-brand-bg px-6 md:px-12 lg:px-24 pt-24 pb-16 perspective-1000 transition-all duration-1000 ease-in-out`}
    >
                    ...
                    <div className="relative bg-white/60 backdrop-blur-2xl border border-white/60 p-2 rounded-[2rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] flex flex-col md:flex-row gap-2">
                      ...
                      <div className="bg-white/50 rounded-[1.5rem] p-6 md:p-8 flex-[1.6] flex flex-col justify-center items-center border border-black/[0.03] shadow-inner relative overflow-hidden">
                        ...
                        <div className="bg-gray-100/50 rounded-[1.5rem] p-6 md:px-10 flex flex-col justify-center items-center border border-black/[0.03] w-full md:w-auto md:min-w-[260px] relative">
                          ...
                          <div className="mt-4 md:absolute md:bottom-3 text-[9px] text-gray-300 font-bold uppercase tracking-[0.2em]">Don't Miss Out</div>
                        </div>
                    </div>
                  </div>
